...
function renderDutyDepartments(departments) {
  ...
  if (!departments || !departments.length) {
    ...
    empty.textContent = "No departments on duty.";
  ...
  departments.forEach(function (dept) {
    ...
    name.textContent = dept.label || "Unknown";

    var count = document.createElement("span");
    count.className = "duty-count";
    count.textContent = (dept.count != null ? dept.count : 0);
...
function handleDutyStatsMessage(data) {
  ...
  if (data.departments && Array.isArray(data.departments)) {
    renderDutyDepartments(data.departments);
  }
}
